...
REM
REM Usage: build-apk-fast.bat [debug|release]
REM Output: release-builds/KushCloud-v<version>.apk
...
if "%BUILD_TYPE%"=="release" (
    ...
    if exist "app\build\outputs\apk\release\app-release-unsigned.apk" (
        copy "app\build\outputs\apk\release\app-release-unsigned.apk" "..\%OUTPUT_DIR%\%APP_NAME%-%VERSION%-release-unsigned.apk"
        echo.
        echo   [!] Release APK is UNSIGNED.
        echo   To sign for Play Store:
        echo   1. jarsigner -verbose -sigalg SHA256withRSA -digestalg SHA256 -keystore my-release-key.jks %OUTPUT_DIR%\%APP_NAME%-%VERSION%-release-unsigned.apk alias_name
        echo   2. zipalign -v 4 %OUTPUT_DIR%\%APP_NAME%-%VERSION%-release-unsigned.apk %OUTPUT_DIR%\%APP_NAME%-%VERSION%-release-signed.apk
        echo.
    ...
    if exist "app\build\outputs\apk\debug\app-debug.apk" (
        copy "app\build\outputs\apk\debug\app-debug.apk" "..\%OUTPUT_DIR%\%APP_NAME%-%VERSION%.apk"
