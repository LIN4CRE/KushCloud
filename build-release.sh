...
# Output: release-[version]/KushCloud-v[version]*.apk
...
# Extract version from package.json for consistency
...
if [ -f "$APK_DEBUG" ]; then
    cp "$APK_DEBUG" "../$RELEASE_DIR/${APP_NAME}-v${VERSION}.apk"
...
if [ -f "$APK_RELEASE" ]; then
    cp "$APK_RELEASE" "../$RELEASE_DIR/${APP_NAME}-v${VERSION}-release-unsigned.apk"
...
if command -v xcodebuild &> /dev/null; then
    echo "   iOS build environment (Xcode) detected."
    echo "   To build iOS: npx cap sync ios && npx cap open ios"
else
    echo "   iOS build requires macOS + Xcode."
